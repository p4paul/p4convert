...
set       toc,title
  ...
  <xsl:param name="monospace.font.family">TheSansMono, monospace</xsl:param>
  <xsl:param name="unicode.font.family">Arial Unicode MS</xsl:param>
  ...
  <!-- workaround for page break bug; apply a large-enough bottom margin -->
  ...
  <!-- adjust orderedlist label width -->
  ...
  <xsl:param name="column.gap.index">24pt</xsl:param>
  ...
  <xsl:template name="book.titlepage.recto">
    <xsl:choose>
      ...
      <xsl:when test="d:title">
        ...
        <fo:block-container position="absolute" top="-18pt" left="2pt" width="160pt" height="40pt">
  ...
  <!-- add a processing instruction to introduce a line break -->
  <xsl:template match="processing-instruction('asciidoc-br')">
    <fo:block/>
  </xsl:template>

  <!-- add a processing instruction to introduce a 'chapter' break
       which should make following content appear on a recto page.
  -->
  <xsl:template match="processing-instruction('chapterbreak')">
    <fo:block page-break-after='right'/>
  </xsl:template>

  <!-- provide a way to remove specific sections from the TOC -->
  <xsl:template match="d:section[@role = 'notoc']"  mode="toc" />

  <!-- custom index.titlepage.recto -->
  <xsl:template name="index.titlepage.recto">
    <fo:block xmlns:fo="http://www.w3.org/1999/XSL/Format"
    xsl:use-attribute-sets="chapter.titlepage.recto.style" margin-top="-36pt">
      <fo:block xsl:use-attribute-sets="pgwide.properties">
        <fo:table xsl:use-attribute-sets="chapter.table.properties">
          <xsl:if test="string($chapter.autolabel) != 0">
            <fo:table-column column-number="1">
  ...
  <!-- customize the keycap template to use a Unicode font for
       keycaps with functions so that special keys like OSX Command
       appear correctly. -->
  <xsl:template match="d:keycap">
    <xsl:choose>
      <xsl:when test="@function and normalize-space(.) = ''">
        <fo:inline font-family="{$unicode.font.family}">
          <xsl:call-template name="inline.boldseq">
            <xsl:with-param name="content">
              <xsl:call-template name="gentext.template">
