...
set       toc,title
  ...
  <!-- turn off the br tag after formal examples. -->
  <xsl:param name="formal.object.break.after">0</xsl:param>

  <!-- PDF availability -->
  <xsl:param name="pdf.available"></xsl:param>

  <!-- Perforce Google Analytics toggle -->
...
<xsl:call-template name="html.head">
  ...
  <body>
    <a id="page-top"></a>
<xsl:if test="$perforce.analytics != '0'">
    <noscript>
      <iframe src="//www.googletagmanager.com/ns.html?id=GTM-PJBHZV" height="0" width="0" style="display:none;visibility:hidden"></iframe>
    </noscript>
    <script>
      <xsl:text>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&amp;l='+l:'';j.async=true;j.src='//www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);})(window,document,'script','dataLayer','GTM-PJBHZV');</xsl:text>
    </script>
</xsl:if>
    ...
    <div id="header">
      <div class="container">
        ...
        <xsl:if test="$pdf.available != ''">
          <a title="Download a PDF version of this guide" class="pdf">
            <xsl:attribute name="href">
              <xsl:value-of select="$pdf.available"/>
            </xsl:attribute>
            <span class="glyphicon glyphicon-book"></span>
          </a>
        </xsl:if>
        <xsl:if test="$webhelp.include.search.tab != '0'">
          <button name="search" type="button" class="search" title="Search this guide">
  ...
  <xsl:template name="webhelpfooter">
    ...
    <xsl:if test="$webhelp.include.search.tab != '0'">
      <script defer="1" type="text/javascript" src="js/index.js"></script>
    </xsl:if>
    <script defer="1" type="text/javascript" src="js/toc.js"></script>
    <script defer="1" type="text/javascript" src="js/perforce.js"></script>
  ...
  <!-- attempt to write toc as JSON file -->
  <xsl:template match="/">
    ...
    <xsl:choose>
      ...
      <xsl:when test="namespace-uri(*[1]) != 'http://docbook.org/ns/docbook'">
        <xsl:call-template name="log.message">
          ...
          <xsl:with-param name="context-desc"><xsl:text>namesp. add</xsl:text></xsl:with-param>
          <xsl:with-param name="message"><xsl:text>added namespace before processing</xsl:text></xsl:with-param>
        </xsl:call-template>
        ...
        <xsl:if test="$collect.xref.targets != 'only'">
          <xsl:call-template name="write.text.chunk">
            <xsl:with-param name="filename">
              ...
              <xsl:value-of select="'js/toc.js'"/>
            </xsl:with-param>
            <xsl:with-param name="method" select="'html'"/>
            <xsl:with-param name="content">
              <xsl:call-template name="jsontoc">
  ...
  <!-- initiates production of a JSON structure representing the TOC -->
  <xsl:template name="jsontoc">
    ...
    <xsl:choose>
      ...
      <xsl:otherwise>
        ...
        <xsl:text>var sidebar_toc = {
  n: [
</xsl:text>
            <xsl:apply-templates select="/*/*" mode="jsontoc">
  ...
  <!-- Generates the entries for the JSON TOC. -->
  <xsl:template match="d:book|d:part|d:reference|d:preface|d:chapter|d:bibliography|d:appendix|d:article|d:topic|d:glossary|d:section|d:simplesect|d:sect1|d:sect2|d:sect3|d:sect4|d:sect5|d:refentry|d:colophon|d:bibliodiv|d:index|d:setindex"
    ...
    <xsl:if test="not(self::index) or (self::index and not($generate.index = 0))">
      ...
      <xsl:value-of select="$indent"/>
      <xsl:text>{ u: "</xsl:text>
      <xsl:value-of select="substring-after($href, $base.dir)"/>
      <xsl:text>", t: "</xsl:text>
  ...
  <!-- disables any elements not covered in the JSON TOC -->
  <xsl:template match="*" mode="jsontoc"/>
